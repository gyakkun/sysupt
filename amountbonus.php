...
//		write_log("25.0 bonus point is sent to everyone. -" . $CURUSER["username"] ,'normal');
    ...
    /*	if ( $seedbonus == 0 )
        {
            stderr("错误！", "魔力值未变动");
            header("Location: /userdetails.php?id=".htmlspecialchars($arr[0]));
            die;
        }
    */
...
?>
    <tr>
        <td class="rowhead" width="100px">用户名：</td>
        <td class="rowfollow"><input type="text" name="username" size="20"/></td>
    </tr>
    <tr>
        <td class="rowhead">魔力值调整量：</td>
        <td class="rowfollow"><input type="text" name="seedbonus" size="5"/></td>
    </tr>
    <tr>
        <td class="rowhead">原因：</td>
        <td class="rowfollow"><textarea rows="10" name="reason" cols="60"></textarea></td>
    </tr>
    <tr>
        <td colspan="2" class="toolbox" align="center"><input type="submit" value="就这样！" class="btn"/></td>
    </tr>
<?php end_table(); ?>
    </form>
<?php end_main_frame(); ?>
<?php begin_main_frame("给所有用户发放魔力值", false, 30); ?>
    <form action="amountbonus.php" method="post">
        <table width=100% border=1 cellspacing=0 cellpadding=5>
            <tr>
                <td class="rowhead">魔力值改变量</td>
                <td><input type="text" name="seedbonus2" size="28"></td>
            </tr>
            <tr>
                <td class="rowhead">原因</td>
                <td class="rowfollow"><textarea name="seedreason" wrap="virtual" rows="3" cols="28"></textarea></td>
            </tr>
            <tr>
                <td class="toolbox" colspan="2" align="center"><input type="hidden" name="doit" value="yes"/>
